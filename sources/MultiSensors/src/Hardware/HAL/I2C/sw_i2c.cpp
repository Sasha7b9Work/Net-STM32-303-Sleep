...
{
    if(out)
        d->hal_io_ctl(HAL_IO_OPT_SET_SDA_HIGH);
    else
        d->hal_io_ctl(HAL_IO_OPT_SET_SDA_LOW);
}

static void i2c_clk_data_out(sw_i2c_t *d)
{
    d->hal_io_ctl(HAL_IO_OPT_SET_SCL_HIGH);
    d->hal_delay_us(SW_I2C_WAIT_TIME);
    d->hal_io_ctl(HAL_IO_OPT_SET_SCL_LOW);
}
...
{
    return (uint8)d->hal_io_ctl(HAL_IO_OPT_GET_SDA_LEVEL);
}
...
static uint8_t SW_I2C_ReadVal_SCL(sw_i2c_t *d)
{
    ...
    return d->hal_io_ctl(HAL_IO_OPT_GET_SCL_LEVEL);
}
#endif
...
static uint8_t i2c_check_ack(sw_i2c_t *d)
{
    ...
    int i;
    unsigned int temp;
    d->hal_io_ctl(HAL_IO_OPT_SET_SDA_INPUT);
    d->hal_io_ctl(HAL_IO_OPT_SET_SCL_HIGH);
    ack = 0;
    d->hal_delay_us(SW_I2C_WAIT_TIME);
    for (i = 10; i > 0; i--)
    ...
    d->hal_io_ctl(HAL_IO_OPT_SET_SCL_LOW);
    d->hal_io_ctl(HAL_IO_OPT_SET_SDA_OUTPUT);
    d->hal_delay_us(SW_I2C_WAIT_TIME);
    return ack;
}
